...
/** Ícones do menu pill — herdam cor do item colorido */
const SECTION_RAIL_SHELL = 'section-icon--rail ring-1 ring-inset bg-transparent'

interface SectionIconProps {
  section: AppSectionId | 'admin'
  size?: keyof typeof SIZE
  active?: boolean
  /** Menu pill da home — flat, sem sombra */
  flat?: boolean
  className?: string
}
...
export function SectionIcon({
  ...
  const shell = cn(
    ...
    SIZE[size],
    flat ? SECTION_RAIL_SHELL : active ? SECTION_ACTIVE_SHELL[section] : SECTION_SHELL[section],
    active && !flat && 'section-icon-active scale-[1.05]',
    flat && 'scale-[1.02]',
    className,
  )
