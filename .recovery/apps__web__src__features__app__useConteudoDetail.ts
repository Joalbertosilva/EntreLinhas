...
import { celebrateReadingProgressChange } from '@/features/app/progressCelebration'
...
export function useAtualizarLeitura(conteudoId: string, usuarioId: string | undefined) {
  ...
  return useMutation({
    ...
    onSuccess: async (status_leitura) => {
      const prevProgress = usuarioId
        ? queryClient.getQueryData<AlunoProgressStats>(['aluno-progress', usuarioId])
        : undefined
      ...
      queryClient.invalidateQueries({ queryKey: ['aluno-progress', usuarioId] })
      if (usuarioId) {
        await celebrateReadingProgressChange(queryClient, usuarioId, status_leitura, prevProgress)
      }
    },
  })
}
